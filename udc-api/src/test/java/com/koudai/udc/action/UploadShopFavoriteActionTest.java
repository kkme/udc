...
public class UploadShopFavoriteActionTest extends TestCase {

	private UploadShopFavoriteAction action;
	...
	private static final String IN = "{\"shopfavorite\":[{\"machineID\":0,\"userID\":0,\"shopID\":0},{\"machineID\":1,\"userID\":1,\"shopID\":1}]}";

	protected void setUp() throws Exception {
		action = new UploadShopFavoriteAction();
	...
	public void testReturnSuccessWhenProductIdIsNull() throws Exception {
		action.setUpload_shopfavorite_in("{\"shopfavorite\":[{\"machineID\":0,\"userID\":0,\"shopID\":\"\"}]}");
		replay();
		assertEquals(ActionSupport.SUCCESS, action.execute());
		assertTrue(action.getResult().toString().contains("\"status_code\":0"));
		verify();
	...
	public void testReturnSuccessWhenProductIdIsEmpty() throws Exception {
		action.setUpload_shopfavorite_in("{\"shopfavorite\":[{\"machineID\":0,\"userID\":0}]}");
		replay();
		assertEquals(ActionSupport.SUCCESS, action.execute());
		assertTrue(action.getResult().toString().contains("\"status_code\":0"));
		verify();
