...
public class UploadProductCollectActionTest extends TestCase {

	private UploadProductCollectAction action;
	...
	private static final String IN = "{\"productcollect\":[{\"machineID\":0,\"userID\":0,\"productID\":0},{\"machineID\":1,\"userID\":1,\"productID\":1}]}";

	protected void setUp() throws Exception {
		action = new UploadProductCollectAction();
	...
	public void testReturnSuccessWhenProductIdIsNull() throws Exception {
		action.setUpload_productcollect_in("{\"productcollect\":[{\"machineID\":0,\"userID\":0,\"productID\":\"\"}]}");
		replay();
		assertEquals(ActionSupport.SUCCESS, action.execute());
		assertTrue(action.getResult().toString().contains("\"status_code\":0"));
		verify();
	...
	public void testReturnSuccessWhenProductIdIsEmpty() throws Exception {
		action.setUpload_productcollect_in("{\"productcollect\":[{\"machineID\":0,\"userID\":0}]}");
		replay();
		assertEquals(ActionSupport.SUCCESS, action.execute());
		assertTrue(action.getResult().toString().contains("\"status_code\":0"));
		verify();
