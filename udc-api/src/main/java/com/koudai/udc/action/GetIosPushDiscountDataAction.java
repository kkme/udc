...
public class GetIosPushDiscountDataAction extends ActionSupport {

	private static final long serialVersionUID = 7870319635340636143L;

	private static final Logger LOGGER = Logger.getLogger(GetIosPushDiscountDataAction.class);
	...
	public String execute() throws Exception {
		try {
			...
			LOGGER.info("Get ios push info with start pos < " + start + " > and end pos < " + end + " > and createTime < " + new DateFormatter().format(createTime) + " >");
			List<IosPushInfo> dbPushInfos = iosPushInfoDAOW.getPushInfosByCreateTimeAndStartAndEndPos(createTime, start, end);
			...
			LOGGER.info("getIosPushDiscountData cost>>>" + costTime);
			return SUCCESS;
		} catch (Exception e) {
			LOGGER.error(e);
